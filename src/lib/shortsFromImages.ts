...
// 아래 이야기 뼈대(훅 / 전개 / 의심 / 반전 / 마무리)가 최소 다섯 박자라서, 네 장면
// 아래로 내려가면 그중 하나를 버려야 하고 그 순간 훅만 있고 이야기가 없는 영상이 된다.
...
- 【리듬】 전부 비슷한 길이면 읽을 때 리듬이 죽어서 기계가 읽는 것처럼 들린다. 숫자로 지킬 것:
  ...
  · 가장 긴 줄은 20자 이상, 25자 이하여야 한다. 30자를 넘는 줄이 있으면 실패다.
  · 즉 가장 긴 줄과 가장 짧은 줄의 차이가 12자 이상이어야 한다. 모든 줄이 10~15자 언저리로 고르게 나오면 그건 실패다.
  · 한 장면에 문장을 두 개 넣지 마라. 넘치면 다음 장면으로 쪼개라.
