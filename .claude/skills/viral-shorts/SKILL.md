...
AI로 만든 것뿐이다. 실행하면 먼저 어떤 영상인지 묻는다(해당 없으면 만들지 않는다).
허락 기록은 사용자가 따로 관리한다.

## 영상 받기 (영상받기.bat = video_download.py)
원작자에게 허락받은 영상을 링크(인스타 릴스·게시물, 유튜브, 틱톡)로 `out/reference/<날짜>/`에
받는다. yt-dlp를 쓰고, 인스타 로그인이 필요한 영상은 브라우저 로그인 정보를 빌린다
(파이어폭스 추천). 옆에 원작자·원래 주소를 담은 .json을 남겨 영상으로쇼츠에서 원작자
표시를 자동으로 채운다. 인스타가 구조를 바꿔 막히면 yt-dlp를 한 번 최신으로 올리고
다시 시도한다(삭제·로그인 필요 같은 실패는 업데이트를 건너뛴다).
